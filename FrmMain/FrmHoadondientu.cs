...
namespace FrmMain
{
    public partial class FrmInvoiceList : FrmBasePos, IReloadableForm
    {
        ...
        public FrmInvoiceList(FrmMainF mainForm, IKiotVietService kiotVietService, IBranchService branchService,
        ...
        private async Task LoadData(string code, long id)
        {
            try
            {
                ...
                if (!success || string.IsNullOrWhiteSpace(content))
                {
                    ...
                    grdControlInvoiceList.DataSource = null;
                    return;
                }
                ...
                grdControlInvoiceList.DataSource = purchaseOrderPagedData.Data;
                grdViewInvoiceList.BestFitColumns();
            }
            catch (Exception ex)
            {
        ...
        private void FrmOrder_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                grdViewInvoiceList.MouseMove += grdViewOrders_MouseMove;
            }
            catch (Exception exception)
            {
                MessageHelper.MsgBox(this,$"Có lỗi trong quá trình tải dữ liệu: {exception}", MsgType.Error);
        ...
        private void SetControlEnable(bool enable)
        {
            ...
            grdControlInvoiceList.Enabled = enable;
        }

        private void InitGridView()
        {
            grdViewInvoiceList.OptionsDetail.EnableMasterViewMode = false;
            grdViewInvoiceList.Columns["PurchaseDate"].DisplayFormat.FormatType = DevExpress.Utils.FormatType.DateTime;
            grdViewInvoiceList.Columns["PurchaseDate"].DisplayFormat.FormatString = "dd/MM/yyyy HH:mm:ss";
        }
        ...
        private void grdViewOrders_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                grdControlInvoiceList.Cursor = Cursors.Hand;
            }
            else
            {
                grdControlInvoiceList.Cursor = Cursors.Default;
            }
        }
        private async void rpBtnAction_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            try
            {
                if (grdViewInvoiceList.FocusedRowHandle < 0) return;

                var orderCode = grdViewInvoiceList.GetRowCellValue(grdViewInvoiceList.FocusedRowHandle, "Code")?.ToString();
                var orderId = grdViewInvoiceList.GetRowCellValue(grdViewInvoiceList.FocusedRowHandle, "Id")?.ToString();
