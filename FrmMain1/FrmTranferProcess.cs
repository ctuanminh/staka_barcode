...
namespace FrmMain
{
    public partial class FrmTranferProcess : XtraForm
    {
        ...
        public FrmTranferProcess(IKiotVietService kiotVietService, IProductService productService, IUserService userService)
        {
            ...
            try
            {
                ...
                switch ((OrderStatusEnum)transferResponse.Status)
                {
                    case OrderStatusEnum.Finished:
                        MessageHelper.MsgBox($"Phiếu: {transferResponse.Code} đã Nhận hàng", MsgType.Error_);
                        txtProductCode.ReadOnly = true;
                        break;
                    case OrderStatusEnum.Cancel:
                        MessageHelper.MsgBox($"Phiếu: {transferResponse.Code} đã Huỷ", MsgType.Error_);
                        txtProductCode.ReadOnly = true;
                        break;
                ...
                var userTranfer = await _userService.GetUserById(transferResponse.CreatedById);
                txtToBranchName.Text = transferResponse.ToBranchName;
                txtTranferName.Text = userTranfer?.FullName;
                txtCountProduct.Text = transferResponse.Count.ToString();
